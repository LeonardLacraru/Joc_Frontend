...
try {
  const saved = JSON.parse(localStorage.getItem("gathering_timer"));
...
const saveTimer = () => {
  try {
    localStorage.setItem("gathering_timer", JSON.stringify(wsManager.timer));
  } catch (e) {}
};
...
// open websocket for gathering
const openTimerWebsocket = (token) => {
  if (!token) return;
  if (wsManager.ws) return; // already open

  const base = (API_BASE_WS || "").replace(/\/+$/, "");
  const wsUrl = `${base}/timer/?token=${encodeURIComponent(token)}&activity=gathering`;
...
onMounted(async () => {
  ...
  // Always check for gathering results on mount
  await getGatheringTask();
});
</script>
      ...
      <div class="card bg-dark text-light">
        ...
        <div v-show="!isResultMinimized" class="card-body">
          <div class="reward-info">
            ...
            <p v-if="gatheringResult.gold !== undefined" class="mb-0 gold-amount">
